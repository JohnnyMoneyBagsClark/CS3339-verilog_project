// Verilated -*- C++ -*-
// DESCRIPTION: Verilator output: Model implementation (design independent parts)

#include "Vaddition__pch.h"
#include "verilated_vcd_c.h"

//============================================================
// Constructors

Vaddition::Vaddition(VerilatedContext* _vcontextp__, const char* _vcname__)
    : VerilatedModel{*_vcontextp__}
    , vlSymsp{new Vaddition__Syms(contextp(), _vcname__, this)}
    , a{vlSymsp->TOP.a}
    , b{vlSymsp->TOP.b}
    , carry_in{vlSymsp->TOP.carry_in}
    , sum{vlSymsp->TOP.sum}
    , carry_out{vlSymsp->TOP.carry_out}
    , rootp{&(vlSymsp->TOP)}
{
    // Register model with the context
    contextp()->addModel(this);
    contextp()->traceBaseModelCbAdd(
        [this](VerilatedTraceBaseC* tfp, int levels, int options) { traceBaseModel(tfp, levels, options); });
}

Vaddition::Vaddition(const char* _vcname__)
    : Vaddition(Verilated::threadContextp(), _vcname__)
{
}

//============================================================
// Destructor

Vaddition::~Vaddition() {
    delete vlSymsp;
}

//============================================================
// Evaluation function

#ifdef VL_DEBUG
void Vaddition___024root___eval_debug_assertions(Vaddition___024root* vlSelf);
#endif  // VL_DEBUG
void Vaddition___024root___eval_static(Vaddition___024root* vlSelf);
void Vaddition___024root___eval_initial(Vaddition___024root* vlSelf);
void Vaddition___024root___eval_settle(Vaddition___024root* vlSelf);
void Vaddition___024root___eval(Vaddition___024root* vlSelf);

void Vaddition::eval_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+++++TOP Evaluate Vaddition::eval_step\n"); );
#ifdef VL_DEBUG
    // Debug assertions
    Vaddition___024root___eval_debug_assertions(&(vlSymsp->TOP));
#endif  // VL_DEBUG
    vlSymsp->__Vm_activity = true;
    vlSymsp->__Vm_deleter.deleteAll();
    if (VL_UNLIKELY(!vlSymsp->__Vm_didInit)) {
        vlSymsp->__Vm_didInit = true;
        VL_DEBUG_IF(VL_DBG_MSGF("+ Initial\n"););
        Vaddition___024root___eval_static(&(vlSymsp->TOP));
        Vaddition___024root___eval_initial(&(vlSymsp->TOP));
        Vaddition___024root___eval_settle(&(vlSymsp->TOP));
    }
    VL_DEBUG_IF(VL_DBG_MSGF("+ Eval\n"););
    Vaddition___024root___eval(&(vlSymsp->TOP));
    // Evaluate cleanup
    Verilated::endOfEval(vlSymsp->__Vm_evalMsgQp);
}

//============================================================
// Events and timing
bool Vaddition::eventsPending() { return false; }

uint64_t Vaddition::nextTimeSlot() {
    VL_FATAL_MT(__FILE__, __LINE__, "", "%Error: No delays in the design");
    return 0;
}

//============================================================
// Utilities

const char* Vaddition::name() const {
    return vlSymsp->name();
}

//============================================================
// Invoke final blocks

void Vaddition___024root___eval_final(Vaddition___024root* vlSelf);

VL_ATTR_COLD void Vaddition::final() {
    Vaddition___024root___eval_final(&(vlSymsp->TOP));
}

//============================================================
// Implementations of abstract methods from VerilatedModel

const char* Vaddition::hierName() const { return vlSymsp->name(); }
const char* Vaddition::modelName() const { return "Vaddition"; }
unsigned Vaddition::threads() const { return 1; }
void Vaddition::prepareClone() const { contextp()->prepareClone(); }
void Vaddition::atClone() const {
    contextp()->threadPoolpOnClone();
}
std::unique_ptr<VerilatedTraceConfig> Vaddition::traceConfig() const {
    return std::unique_ptr<VerilatedTraceConfig>{new VerilatedTraceConfig{false, false, false}};
};

//============================================================
// Trace configuration

void Vaddition___024root__trace_decl_types(VerilatedVcd* tracep);

void Vaddition___024root__trace_init_top(Vaddition___024root* vlSelf, VerilatedVcd* tracep);

VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    // Callback from tracep->open()
    Vaddition___024root* const __restrict vlSelf VL_ATTR_UNUSED = static_cast<Vaddition___024root*>(voidSelf);
    Vaddition__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    if (!vlSymsp->_vm_contextp__->calcUnusedSigs()) {
        VL_FATAL_MT(__FILE__, __LINE__, __FILE__,
            "Turning on wave traces requires Verilated::traceEverOn(true) call before time 0.");
    }
    vlSymsp->__Vm_baseCode = code;
    if (strlen(vlSymsp->name())) tracep->pushPrefix(std::string{vlSymsp->name()}, VerilatedTracePrefixType::SCOPE_MODULE);
    Vaddition___024root__trace_decl_types(tracep);
    Vaddition___024root__trace_init_top(vlSelf, tracep);
    if (strlen(vlSymsp->name())) tracep->popPrefix();
}

VL_ATTR_COLD void Vaddition___024root__trace_register(Vaddition___024root* vlSelf, VerilatedVcd* tracep);

VL_ATTR_COLD void Vaddition::traceBaseModel(VerilatedTraceBaseC* tfp, int levels, int options) {
    (void)levels; (void)options;
    VerilatedVcdC* const stfp = dynamic_cast<VerilatedVcdC*>(tfp);
    if (VL_UNLIKELY(!stfp)) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vaddition::trace()' called on non-VerilatedVcdC object;"
            " use --trace-fst with VerilatedFst object, and --trace with VerilatedVcd object");
    }
    stfp->spTrace()->addModel(this);
    stfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vaddition___024root__trace_register(&(vlSymsp->TOP), stfp->spTrace());
}
